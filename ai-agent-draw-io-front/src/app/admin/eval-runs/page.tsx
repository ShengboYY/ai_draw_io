...
export default function AdminEvalRunsPage() {
  ...
  useEffect(() => {
    agentApi.adminListEvalDatasets().then(({ data }) => setDatasets(data || [])).catch((reason) => setError(reason instanceof Error ? reason.message : 'Failed to load Datasets'));
    agentApi.adminListEvaluationProfiles().then(({ data }) => setProfiles(data || [])).catch((reason) => setError(reason instanceof Error ? reason.message : 'Failed to load Evaluation Profiles'));
  }, []);
  const update = (field: keyof RunDraft, value: string) => setDraft((current) => ({ ...current, [field]: value }));
  const selectDataset = (datasetId: string) => {
    setDraft((current) => ({ ...current, datasetId, datasetVersion: '', profileKey: '' }));
    setDatasetVersions([]);
  ...
  const start = () => {
    ...
    if (!draft.datasetId || !draft.datasetVersion || !draft.gitSha || !profile) { setError('Dataset, published version, Evaluation Profile and candidate Git SHA are required.'); return; }
  ...
  return <AdminShell active="evalRuns"><AdminPageHeading eyebrow="Evaluation · Step 3" title="Run and inspect the Agent"
    ...
    {showForm && <section className="mb-5 rounded-lg border border-stone-200 bg-white p-4"><div className="grid gap-3 md:grid-cols-4">
      <label className="text-xs text-zinc-500">Mode<select value={draft.mode} onChange={(event) => setDraft((current) => ({ ...current, mode: event.target.value as RunMode, profileKey: '' }))} className="mt-1 w-full rounded border border-stone-200 p-2 text-sm"><option value="MODE_B">Mode B · recorded replay</option><option value="MODE_C">Mode C · live model</option><option value="RELEASE">Release · baseline comparison</option></select></label>
      <FormSelect label="Dataset" value={draft.datasetId} placeholder="Select Dataset…" options={datasets.map((dataset) => ({ value: dataset.id, label: `${dataset.name} · ${dataset.datasetClass} · ${dataset.evaluationTarget?.replaceAll('_', ' ') || 'Target pending'}` }))} onChange={selectDataset} />
      ...
      <FormSelect label="Evaluation Profile" value={draft.profileKey} placeholder={datasetTarget ? 'Select compatible Profile…' : 'Choose Dataset first'} disabled={!datasetTarget} options={compatibleProfiles.map((profile) => ({ value: `${profile.profileId}@${profile.version}`, label: `${profile.profileId}@${profile.version} · ${profile.repetitions}×` }))} onChange={(value) => update('profileKey', value)} />
    ...
    <div className="space-y-3">{!error && runs.length === 0 && <div className="rounded-xl border border-dashed border-stone-300 bg-stone-50 px-5 py-10 text-center"><p className="text-sm font-medium text-zinc-700">No evaluation history yet</p><p className="mt-1 text-xs text-zinc-500">Publish a Dataset, then start a Mode B run to get your first repeatable baseline.</p></div>}{runs.filter((run) => !targetFilter || run.evaluationTarget === targetFilter).map((run) => <Link key={run.id} href={`/admin/eval-runs/${encodeURIComponent(run.id)}`} className="block rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3"><div><div className="font-medium">{run.datasetId}@{run.datasetVersion}</div><div className="mt-1 font-mono text-[11px] text-zinc-400">{run.evaluationTarget?.replaceAll('_', ' ') || 'Target unresolved'} · {run.profileId ? `${run.profileId}@${run.profileVersion}` : 'Legacy Profile'} · {run.mode} · {run.gitSha} · {run.id}</div></div><Status value={run.status} /></div>
