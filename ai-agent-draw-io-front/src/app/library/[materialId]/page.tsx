...
export default function MaterialDetailsPage() {
  ...
  useEffect(() => {
    ...
    materialClient.pageSet(materialId, versionId)
      .then(setPageSet)
      .catch(() => setMessage('Page previews are currently unavailable.'));
  ...
  const reprocess = async () => {
    try {
      ...
      setMessage('The reprocessing request has been submitted.');
    } catch {
      setMessage(materialAccessMessage('HTTP_ERROR'));
    }
  };
  ...
  const permanentlyDelete = async () => {
    try {
      ...
      const confirmation = window.prompt(`This will permanently delete ${impact.versionCount} versions and affect ${impact.citationCount} citations. Type DELETE to continue.`);
  ...
  return (
    <main className="app-page min-h-screen bg-stone-50 text-zinc-800"><div className="mx-auto max-w-6xl px-5 py-7">
      <Link href="/library" className="text-sm font-medium text-zinc-600 hover:underline">← Back to Library</Link>
      ...
      {details && <div className="mt-5 space-y-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <div><label className="mb-2 block text-sm font-medium">Version</label><select value={versionId} onChange={event => setVersionId(event.target.value)} className="w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm">{details.versions.map(version => <option key={version.versionId} value={version.versionId}>Version {version.versionNo} · {version.processingStatus}</option>)}</select>
            {selectedVersion && <p className="mt-2 text-sm text-zinc-500">{selectedVersion.detectedMime} · {selectedVersion.pageCount || 0} {selectedVersion.pageCount === 1 ? 'page' : 'pages'} · {selectedVersion.progress}%</p>}
            ...
            {pageSet && <ul className="mt-4 grid gap-2 sm:grid-cols-2" aria-label="Page processing status">{pageSet.pages.map(page => <li key={page.pageNo} className="rounded-lg border border-stone-200 bg-white p-3 text-sm"><div className="flex items-center justify-between"><span>Page {page.pageNo}</span><label className="flex items-center gap-1 text-xs"><input type="checkbox" checked={pageSet.excludedPages.includes(page.pageNo)} onChange={event => void replaceExcludedPage(page.pageNo, event.target.checked)} /> Exclude</label></div><p className="mt-2 text-xs text-zinc-500">Native text: {page.nativeTextStatus} · OCR: {page.ocrStatus} · Visual: {page.visualStatus}</p>{page.errorCode && <p className="mt-1 text-xs text-amber-700">{page.errorCode}</p>}</li>)}</ul>}
          </div>
          <aside className="space-y-5 rounded-xl border border-stone-200 bg-white p-4"><div><h2 className="font-semibold">Item links</h2><p className="mt-1 text-sm text-zinc-500">Linking an item does not duplicate the file.</p><ul className="mt-3 space-y-2">{details.scopes.map(scope => <li key={scope.linkId} className="flex items-center justify-between gap-2 text-sm"><span>{scope.scopeType} · {scope.scopeKey}</span><button type="button" onClick={() => void removeScope(scope.linkId)} className="text-rose-700 underline">Remove</button></li>)}</ul>
            <div className="mt-3 flex gap-2"><input value={scopeId} onChange={event => setScopeId(event.target.value)} placeholder="Chartbook ID" className="min-w-0 flex-1 rounded-lg border border-stone-300 px-2 py-1.5 text-sm"/><button type="button" onClick={() => void addChartbookScope()} className="theme-btn-secondary rounded-lg px-3 text-sm">Link</button></div>
          </div><div className="border-t border-stone-100 pt-4"><button type="button" onClick={() => void reprocess()} className="text-sm font-medium text-zinc-700 underline">Reprocess</button>{details.material.lifecycleState === 'TRASHED' ? <><button type="button" onClick={() => void restore()} className="mt-3 block text-sm font-medium text-zinc-700 underline">Restore item</button><button type="button" onClick={() => void permanentlyDelete()} className="mt-3 block text-sm font-medium text-rose-700 underline">Delete permanently</button></> : <button type="button" onClick={() => void moveToTrash()} className="mt-3 block text-sm font-medium text-rose-700 underline">Move to trash</button>}</div></aside>
        </section>
      </div>}
    </div></main>
  );
