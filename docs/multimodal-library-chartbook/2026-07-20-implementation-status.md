...
- 同步/流式 `AgentConversationService` 通过 `RequestProbeService` 与 `EvidencePreparationModule` 两个检索外观准备资料。流式协议新增 evidence progress，以及 `source_wait_started/source_not_ready/target_clarification/degraded` 完成事件；完成型证据路径不复用 `user` clarification chunk。WP6 接管流式绘图的 `READY`，WP7 接管同步与流式 `answer_with_evidence`；同步绘图仍保持关闭，不会绕过尚未具备的原子提交边界。
...
- 新增 `EvidenceAccessContext` 与 `EvidencePromptAssembler`。Drawer 只看到本轮 `citationKey`、短 source label、页码、modality、`SUPPORT|CONTEXT_ONLY` role 和经过 data delimiter 转义的 display text，不看到 Material/Version/Evidence/S3/Pinecone 内部 identity；`EXPLICIT_ONLY` 会关闭 AI knowledge。Evidence 正文只进入本轮模型上下文，debug trace 仅记录消息 SHA、数量、key、模态和 role。
- Draw.io create/modify contract 增加 `citationBindings`、support atoms、statement kind 与 support type。`CitationGuard` 先验证最终规范化 XML 的 cell/statement/edge 对应、Bundle 白名单、`SUPPORT` role、连续原文 anchor、引用完整性和 strict factual coverage；`CONTEXT_ONLY` 不能持久化为 claim 来源。非完全 extractive statement 再由独立无工具 configured-model agent 批量做 entailment，最多八条，超时、破损 schema、部分支持或冲突全部 fail closed；格式错误的 manifest 与合法空 manifest 分开处理，并且不受 strict 开关影响始终拒绝。
- `CanvasCommitModule` 将 Mutation Gate、Citation Guard、服务端 opaque `zipp*` metadata 注入和一个 MySQL transaction 串成单一提交点。画布当前状态、`diagram_canvas_version`、`source_citation`、`citation_evidence`、cell provenance、source pins 与 grounded run `COMPLETED` 同时成功或同时回滚；模型伪造的 `zipp*` 属性会先被清除。AI 更新时，语义未变化且本轮未重新绑定的 cell 会继承前版 provenance 与 opaque metadata，pin 依据当前画布完整 provenance anchor 集合计算；DOM normalize/reopen 回归测试覆盖属性恢复，真实 diagrams.net 浏览器导出/重开仍需在部署环境做一次 smoke test。
- grounded run 在 retrieval 前写入 durable `RUNNING,generation=1` fence。timeout、断连和完成前取消使用行锁与 generation CAS 转为 `CANCELLED,generation+1`；若提交已完成返回 `ALREADY_COMPLETED`。提交 transaction 必须 `SELECT ... FOR UPDATE` 读取仍为 `RUNNING` 且 generation 相同的 owner/request/run，晚到结果不能自行补建 run 后提交。
- 人工 canvas save 在 feature 开启时通过事务协调器写版本历史：未变化和纯 STYLE/GEOMETRY/WAYPOINTS 的 cell 继承上一版 provenance；文字、端点、父子关系、增删等语义变化写为 `MANUAL`，删除 cell 不再复制 provenance。新导入 cell 会保留格式合法的 opaque ref：只有同一 Owner 且当前语义哈希匹配时才通过原 anchor 恢复 citation set，并为新图表补齐 source pins；跨 Owner 或语义不匹配只保留不可解析 ref 并标为 `UNATTRIBUTED`，不泄露或伪造原来源。
- 新增 owner-fenced cell citation query API，按指定或当前 canvas version 从 provenance 恢复实际引用，并校验当前 XML 携带的 `provenanceRef`，返回来源名、固定版本/processing revision、页码、bbox、modality、origin、最多 800 字符的 bounded excerpt、owner-fenced page preview URL 与当前来源状态。Evidence Unit display text 从本阶段起写为独立 exact-version immutable artifact，不再误读整份 gzip manifest。删除来源只返回 `SOURCE_UNAVAILABLE` 和删除时间，不返回残余名称、版本、页码或区域；Draw.io 选择节点后展示安全来源摘要。未使用的 Evidence Bundle candidates 没有 citation row，因此不会进入面板。
- 新增 `2026-08-07-add-cell-provenance-reference.sql`，将 XML 中 server-owned `zippProvenanceRef` 持久化到 `diagram_cell_provenance.provenance_ref`。style-only/manual 继承保留原 ref，语义变化生成并同时写入新 ref；AI 继承从 owner-fenced DB projection 重建 XML，不信任客户端属性。线上仍需先按顺序执行全部 migration，并完成 `2026-08-06-add-online-retrieval-artifact-identity.sql`、本 migration 和旧 Revision reprocess，旧 Revision 重新处理后才有独立 excerpt artifact；`MATERIAL_RAG_ENABLED` 与 `MATERIAL_LIFECYCLE_ENABLED` 默认关闭，关闭时普通文本绘图不创建这些服务或依赖 Pinecone/S3。
- 涉及图片/图表理解的请求仍要求受控 `VisualEvidenceVerifier`；未配置时保持 WP5 的明确停止，不能以 OCR/caption 猜测视觉事实。

## WP7 交付范围

- 新增 DDD `EvidenceAnswerService`、`EvidenceAnswerGuard`、无工具 `EvidenceAnswerGeneratorPort` 与单事务 `EvidenceAnswerCommitPort`。回答模型只能提出结构化 claim/gap/conflict、Bundle citation key 和连续 support atom，不能提交自由 Markdown；服务端只渲染 Guard 接受的 claim。`DIRECT` 只有每个实际引用都与 statement 精确相等时走本地 fast path，`SYNTHESIZED` 始终复用 WP6 configured-model entailment verifier；当前尚无 authenticated visual observation，因此 `VISUAL_VERIFIED` 明确拒绝，不允许文本伪装视觉核验。越界 key、`CONTEXT_ONLY`、不连续 anchor、坏 schema、缺失 verdict 和 verifier failure 均 fail closed。
- 回答消息、`source_citation(target_type=ANSWER_CLAIM)`、`citation_evidence`、source pin 与 grounded run `COMPLETED` 在同一 MySQL transaction 中提交。事务先锁定 run fence 与 owner-fenced canvas version/hash，只读取且绝不更新画布；tuple 变化会回滚。客户端提供 stable assistant message id，但事务只允许插入，不能覆盖已有 user/agent message。完成型 `evidence_answer` 流事件只在提交成功后发送。
- Target Resolver 的唯一 selection/name 结果会把服务端解析的 cell ID/kind/label 传入回答模型，并只把短 label 用作 query context；label 与 MANUAL cell 永远不是 Evidence Item。若当前 target provenance 为 `EVIDENCE`，在线 catalog 先 hydrate 并判断当前引用是否足够，只有不足时才执行 supplemental retrieval；固定旧 Revision 可继续通过当前授权和 lease 使用。Bundle item 标记 `EXISTING_REFERENCE/EXPLICIT/SEARCH/SUPPLEMENTAL`，UI 区分已有引用与本次补充。`EXPLICIT_ONLY` 仍只查询显式 Version，不扩展图表 pin、图表册或个人资料库。
- 前端请求携带 selection cell IDs 与 authoritative canvas version/hash；同名 target 返回候选和同一 tuple，Draw.io bridge 高亮候选，用户选择后用该 tuple 重发；过期 selection 使用独立事件清除，不能以空候选进入 highlight。回答消息展示服务端来源标签、页码、模态和 origin；claim/source projection 可随历史消息重新加载，且通用消息保存不能覆盖已经绑定 `ANSWER_CLAIM` 的回答。
- 新增 `2026-08-08-add-answer-citation-origin.sql`，只为 `citation_evidence` 增加 source origin，使 `EXISTING_REFERENCE/EXPLICIT/SEARCH/SUPPLEMENTAL` 在刷新后仍可恢复。回答提交事务还会重新校验 Owner、Material 生命周期、固定 Version/Revision、Evidence 状态和当前 run read lease。部署前需按顺序应用至本 migration，并同时启用 `MATERIAL_LIFECYCLE_ENABLED=true` 与 `MATERIAL_RAG_ENABLED=true`；Pinecone dense 仍为独立可选开关，普通文本绘图不依赖这些能力。

## WP8 交付范围

- 新增 DDD `RagBetaReleaseGate`、`RagEvaluationReport` 与全部第 19.5 节 release metrics。Gate 要求至少 165 case、至少 30% locked case、所有指标齐全并逐项达标；`EXPLICIT_ONLY` 越界、跨 Owner 回源、无效生命周期 lease 和扫描前内容投递均为精确零容忍，不做四舍五入。普通文本回归、真实 selection highlight、回答原子提交和 canvas version/hash 不变四项 smoke 也是强制门禁。
- Release report 现在还固定 report/dataset/processing/ranking/model/deployment profile，并生成 `rag-approval-v1:<sha256>` 作为唯一上线 pin；seed dataset 永远不能通过。No-answer 校准报告强制执行 30/40/50/100 的 fallback 样本门槛、raw rate 和置信区间，缺少全局 `N>=100` 时只能 shadow。
- 新增 `rag-eval-case-v1` 合同、JSON loader、三类 seed case、165-case 分类预算和安全 corpus manifest。Seed dataset 使用独立 `rag-beta-v1-seed` 版本，只验证 schema，不能冒充 release dataset。实际 165-case 中英文 synthetic fixtures、50 个 locked case 及其运行结果仍必须由受控 evaluation control plane 发布和执行；代码不会伪造通过报告。
- 新增富领域 `MaterialCapacityBreaker` 与 `MaterialRolloutGate`。70/85/95% 分别进入 warning/restricted/exhausted；85% 停止匿名低优先级 reprocess，95% 停止新的匿名 upload。容量快照不可用时匿名新工作 fail closed，注册用户与普通文本绘图继续可用。匿名上传还必须同时满足所有前序 flag 和固定 release report approval；只打开 `MATERIAL_UPLOAD_ANONYMOUS_ENABLED` 不能绕过门禁。
- 新增只读 `MaterialOperationalSnapshot`、带 25 秒缓存和覆盖索引的 MySQL 聚合 adapter、`MaterialCapabilityService`、低基数 Micrometer metrics 和管理员 capability API。Dashboard 区分普通文本可用与资料能力 disabled/degraded/misconfigured/blocked，报告 processing queue、24h failure、read lease、deletion lag、月度 indexed pages、reconciliation backlog 和 provider capacity；不读取正文、文件名或 owner/material/run ID。Release Owner 可评估 `rag-release-report-v1`；billing exporter 使用独立 service secret、provider capture time 和单调 sequence 更新共享 MySQL 单例快照，迟到重试不能覆盖新容量，所有 ECS task 读取一致状态；容量样本过期时匿名工作 fail closed。
- 在线 retrieval 通过 `MaterialRetrievalTelemetry` 记录 route/source mode/result、耗时和最终 Evidence 数量；metrics tag 会限制为固定低基数值。新增 Prometheus alert rules，覆盖 queue oldest age、70/85/95 capacity、expired lease、stuck deletion、terminal job failure 与 rollout misconfiguration。
- Rollout flag 已拆分为 ingestion、upload/catalog、retrieval shadow、explicit RAG、citation commit、evidence answer、anonymous upload；citation 与 evidence answer bean 默认独立关闭。Shadow flag 通过与主 Intent Router 隔离的异步 observer 执行 source policy、candidate retrieval 和 MySQL reauthorization，到候选授权即停止，不申请 read lease、不 hydration、不写 prompt/citation/响应，也不增加普通绘图等待时间。普通文本路径始终不读取资料 operation snapshot，也不依赖 Worker、ClamAV、S3 或 Pinecone。
- Reconciliation 沿用并纳入 capability/alert 观察的既有 fenced 深模块：Processing/Deletion lease reaper、Material lifecycle/read-lease maintenance、missing vector repair、provider orphan intent/delete 和 retired-generation purge。WP8 没有增加绕过权威表的“全量修复”入口。
- 新增 Beta runbook、fault-injection matrix、Prometheus rules 和隐私披露，明确 scan-before-delivery、exact VersionId 删除、内容不进入 metrics/trace、24h/30d 生命周期、墓碑脱敏、分批开启与 additive rollback。

## 下一阶段

代码工作包已经到 WP8。下一步不是继续增加默认开启的功能，而是在部署环境按顺序应用至 `2026-08-09-add-material-operations-indexes.sql`，接入容量 exporter，执行 Beta release validation：补齐并发布 165-case locked dataset、运行安全 corpus/性能/故障注入、导入告警规则、提交固定 release report，并完成真实 diagrams.net 与 MySQL/Pinecone/S3 的端到端 smoke。所有 gate 通过前保持 `MATERIAL_RELEASE_APPROVED=false` 和 `MATERIAL_UPLOAD_ANONYMOUS_ENABLED=false`。
