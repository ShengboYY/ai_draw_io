...
下一阶段进入 WP8：补齐安全 corpus、RAG/grounding dataset gates、故障注入、metrics/alerts/capability dashboard、capacity breaker、reconciliation、feature flag 分批开启、Beta 运维 runbook 与隐私披露。Beta 前还需在真实 diagrams.net 与部署数据库上完成 selection highlight、回答原子提交和 canvas version/hash 不变的端到端 smoke test。
