...
- 涉及图片/图表理解的请求仍要求受控 `VisualEvidenceVerifier`；未配置时保持 WP5 的明确停止，不能以 OCR/caption 猜测视觉事实。证据回答、selection highlight/同名 target 交互属于 WP7，尚未由本阶段开放。

## 下一阶段

下一阶段进入 WP7：实现 EvidenceAnswerService/Guard、answer claim 与 citation 的原子持久化/渲染、existing-citation-first 与 supplemental source 策略，以及 selection highlight 和同名 target clarification 交互。完成定义仍包括：证据问答不得修改画布 version/hash，MANUAL 内容不能自证，`EXPLICIT_ONLY` 不扩展来源。
