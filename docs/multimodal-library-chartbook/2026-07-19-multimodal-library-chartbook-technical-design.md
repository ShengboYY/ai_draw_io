...
| `material_content_blob` | `id`, `owner_type`, `owner_key`, `content_sha256`, `byte_size`, `detected_mime`, `original_object_key`, `status` | `(owner_type, owner_key, content_sha256, byte_size)` 唯一；只在同 Owner 内物理复用，引用计数为 0 才删 original |
