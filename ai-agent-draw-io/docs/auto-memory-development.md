...
未见 V2 holdout 尚未运行。跨语言的单目标低排名问题仍不属于 V1.8。
...
- [x] V1.8.1 只扩充宽松模型调用信号，并用 semanticKey 通用判据澄清独立意图；development-v2
  首次 10/10 通过，三项质量指标均为 100%、协议失败为 0，未见 V2 holdout 尚未运行。
...
因漏拆未通过；V1.8.1 development-v2 已通过，新 V2 holdout 等待实现固定后一次性运行。单目标
跨语言召回仍是独立问题，不能继续修改全局 score 参数顺带处理。与此同时随本地真实使用积累
非合成样本并比较错误归并，
