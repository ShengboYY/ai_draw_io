...
public class EvalCaseVersionRepository implements IEvalCaseVersionStore {
    ...
    private EvalCaseVersion toDomain(EvalCaseVersionPO po) { return EvalCaseVersion.builder().caseId(po.getCaseId()).caseVersion(po.getCaseVersion()).contentHash(po.getContentHash()).artifactRef(po.getArtifactRef()).approvedBy(po.getApprovedBy()).publishedAt(po.getPublishedAt().toInstant()).retiredAt(po.getRetiredAt() == null ? null : po.getRetiredAt().toInstant()).build(); }
    private EvalCaseVersionPO toPo(EvalCaseVersion value) { EvalCaseVersionPO po = new EvalCaseVersionPO(); po.setCaseId(value.getCaseId()); po.setCaseVersion(value.getCaseVersion()); po.setContentHash(value.getContentHash()); po.setArtifactRef(value.getArtifactRef()); po.setApprovedBy(value.getApprovedBy()); po.setPublishedAt(Date.from(value.getPublishedAt())); po.setRetiredAt(value.getRetiredAt() == null ? null : Date.from(value.getRetiredAt())); return po; }
}
