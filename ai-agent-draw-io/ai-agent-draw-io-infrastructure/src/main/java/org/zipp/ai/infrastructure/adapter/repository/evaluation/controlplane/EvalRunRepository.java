...
public class EvalRunRepository implements IEvalRunStore {
    ...
    private EvalRun run(EvalRunPO po) { return EvalRun.builder().id(po.getId()).mode(EvalRunMode.valueOf(po.getMode())).datasetId(po.getDatasetId()).datasetVersion(po.getDatasetVersion()).baselineRef(po.getBaselineRef()).candidateRef(po.getCandidateRef()).executionProfileHash(po.getExecutionProfileHash()).idempotencyKey(po.getIdempotencyKey()).repetitions(po.getRepetitions()).plannedEpisodes(po.getPlannedEpisodes() == null ? 0 : po.getPlannedEpisodes()).gitSha(po.getGitSha()).graderManifestJson(po.getGraderManifestJson()).reportRef(po.getReportRef()).status(EvalRunStatus.valueOf(po.getStatus())).createdBy(po.getCreatedBy()).createdAt(instant(po.getCreatedAt())).startedAt(instant(po.getStartedAt())).completedAt(instant(po.getCompletedAt())).build(); }
    private EvalRunPO runPo(EvalRun value) { EvalRunPO po = new EvalRunPO(); po.setId(value.getId()); po.setMode(value.getMode().name()); po.setDatasetId(value.getDatasetId()); po.setDatasetVersion(value.getDatasetVersion()); po.setBaselineRef(value.getBaselineRef()); po.setCandidateRef(value.getCandidateRef()); po.setExecutionProfileHash(value.getExecutionProfileHash()); po.setIdempotencyKey(value.getIdempotencyKey()); po.setRepetitions(value.getRepetitions()); po.setPlannedEpisodes(value.getPlannedEpisodes()); po.setGitSha(value.getGitSha()); po.setGraderManifestJson(value.getGraderManifestJson()); po.setReportRef(value.getReportRef()); po.setStatus(value.getStatus().name()); po.setCreatedBy(value.getCreatedBy()); po.setCreatedAt(date(value.getCreatedAt())); po.setStartedAt(date(value.getStartedAt())); po.setCompletedAt(date(value.getCompletedAt())); return po; }
