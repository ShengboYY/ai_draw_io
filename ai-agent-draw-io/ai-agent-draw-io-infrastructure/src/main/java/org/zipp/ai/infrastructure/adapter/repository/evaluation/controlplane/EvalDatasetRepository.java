...
public class EvalDatasetRepository implements IEvalDatasetStore {
    ...
    @Override public void insertDataset(EvalDataset value) { EvalDatasetPO po = new EvalDatasetPO(); po.setId(value.getId()); po.setName(value.getName()); po.setDatasetClass(value.getDatasetClass().name()); po.setEvaluationTarget(value.getEvaluationTarget() == null ? null : value.getEvaluationTarget().name()); po.setOwnerUserId(value.getOwnerUserId()); po.setCreatedAt(Date.from(value.getCreatedAt())); mapper.insertDataset(po); }
    @Override public Optional<EvalDataset> findDataset(String id) { return Optional.ofNullable(mapper.selectDataset(id)).map(this::dataset); }
    @Override public List<EvalDataset> listDatasets() { return mapper.selectDatasets().stream().map(this::dataset).toList(); }
    ...
    @Override @Transactional public boolean validateWithTarget(EvalDatasetVersion value, long revision, EvaluationTarget target) {
        // Update the optimistic-lock protected Version first; an assignment failure rolls the transaction back.
        if (mapper.updateDatasetVersion(toPo(value), revision) != 1) return false;
        if (mapper.assignDatasetTarget(value.getDatasetId(), target.name()) != 1) {
            throw new org.zipp.ai.domain.agent.service.evaluation.controlplane.EvalControlPlaneException(
                    org.zipp.ai.domain.agent.service.evaluation.controlplane.EvalControlPlaneErrorCode.TARGET_MISMATCH,
                    "dataset target assignment conflict");
        }
        return true;
    }
    ...
    private EvalDataset dataset(EvalDatasetPO po) { return EvalDataset.builder().id(po.getId()).name(po.getName()).datasetClass(EvalDatasetClass.valueOf(po.getDatasetClass())).evaluationTarget(target(po.getEvaluationTarget())).ownerUserId(po.getOwnerUserId()).createdAt(po.getCreatedAt().toInstant()).build(); }
    private EvalDatasetVersion toDomain(EvalDatasetVersionPO po) { List<EvalDatasetMember> members = mapper.selectDatasetMembers(po.getDatasetId(), po.getVersion()).stream().map(member -> EvalDatasetMember.builder().caseId(member.getCaseId()).caseVersion(member.getCaseVersion()).build()).toList(); return EvalDatasetVersion.builder().datasetId(po.getDatasetId()).version(po.getVersion()).datasetClass(EvalDatasetClass.valueOf(po.getDatasetClass())).evaluationTarget(target(po.getEvaluationTarget())).status(EvalDatasetVersionStatus.valueOf(po.getStatus())).contentHash(po.getContentHash()).revision(po.getRevision()).members(members).publishedBy(po.getPublishedBy()).publishedAt(po.getPublishedAt() == null ? null : po.getPublishedAt().toInstant()).build(); }
    private EvalDatasetVersionPO toPo(EvalDatasetVersion value) { EvalDatasetVersionPO po = new EvalDatasetVersionPO(); po.setDatasetId(value.getDatasetId()); po.setVersion(value.getVersion()); po.setDatasetClass(value.getDatasetClass().name()); po.setEvaluationTarget(value.getEvaluationTarget() == null ? null : value.getEvaluationTarget().name()); po.setStatus(value.getStatus().name()); po.setContentHash(value.getContentHash()); po.setRevision(value.getRevision()); po.setPublishedBy(value.getPublishedBy()); po.setPublishedAt(value.getPublishedAt() == null ? null : Date.from(value.getPublishedAt())); return po; }
    private EvaluationTarget target(String value) { return value == null ? null : EvaluationTarget.valueOf(value); }
}
