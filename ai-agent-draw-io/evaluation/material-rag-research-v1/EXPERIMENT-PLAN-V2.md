...
| 分区 | 数量 | 用途 |
|---|---:|---|
| Development | 120 | 调试、分析、选择候选方案 |
| Validation | 60 | 从候选中选择晋级方案 |
| Holdout | 60 | 只比较冻结基线和最终方案 |

180 个代表可以开始正式比较，完成结论必须达到 240 个。当前已执行的 38 个案例只作为
历史基线，E0 仍需在冻结后的 240 个案例上重跑。
...
先完成 Pre-E0 的语料建设与冻结，不继续使用当前小型合成资料直接推进 E1：

1. 保留现有短 PDF 作为 controlled regression fixtures，不把它们视为真实文档代表。
2. 补充文章型长文档、复杂表格、图文混排、扫描件、版本冲突和跨文档资料。
3. 先按 document/version/fact family 划分 Development、Validation 和 Holdout，再生成问题。
4. 为每个问题标注 Evidence、required evidence groups、答案、拒答条件和允许的版本/范围。
5. 达到 180 个已复核案例后可以开始正式比较；补齐并冻结 240 个核心案例后运行 E0。
6. E0 完成后再从 E1 开始单变量实验，之前的 E1 中断结果不作为正式比较结果。
