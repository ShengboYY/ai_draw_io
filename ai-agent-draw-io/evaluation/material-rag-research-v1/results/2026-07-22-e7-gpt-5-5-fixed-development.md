...
The visual request-route task (`dgt-dev-02`) and the V1-pinning task (`dgt-dev-04`) completed. The other four outputs had usable XML but missing or nonconforming citations. In particular, the scanned-workshop output cited generated cell IDs rather than the provided evidence anchor. The XML evaluator now normalizes draw.io HTML label formatting before checking required labels; this is formatting normalization, not a relaxation of the required diagram structure.
...
**Decision:** do not promote or open Validation. The fixed-context E7 citation-completeness rate is far below the plan's 0.90 target. This isolates a generation/citation-contract failure; it is not an E6 context-selection result and says nothing about raw top-8 versus selected top-8 quality.
