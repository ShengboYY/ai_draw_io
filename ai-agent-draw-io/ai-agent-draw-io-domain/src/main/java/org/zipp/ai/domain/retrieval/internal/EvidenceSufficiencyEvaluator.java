...
final class EvidenceSufficiencyEvaluator {
    ...
    private static final Set<String> STOP_TERMS = Set.of("please", "according", "based", "source", "sources",
    ...
    Result evaluate(String request, RetrievalRoute route, List<EvidenceBundleItem> items) {
        ...
        if (exacts.stream().anyMatch(token -> !evidence.contains(token))) {
            return Result.unsupported("REQUIRED_EXACT_TERM_MISSING");
        }
        ...
        if (broadSummary) {
            ...
            if (!terms.isEmpty() && matchedTerms == 0) {
                return Result.unsupported("ABSOLUTE_RELEVANCE_TOO_LOW");
            }
            return items.size() >= 2 && characters >= 160
                    ? Result.supported() : Result.unsupported("SUMMARY_COVERAGE_TOO_SMALL");
        }
    ...
    record Result(boolean sufficient, String gap) {
        static Result supported() { return new Result(true, ""); }
        static Result unsupported(String gap) { return new Result(false, gap); }
    }
}
